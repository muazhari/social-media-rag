...
def is_base64_data_uri(s: str) -> bool:
    """
    Check if a string is a base64 data URI (data:mime/type;base64,XXXX).
    Returns True if it's a valid base64 data URI, False otherwise.
    """
...
class CustomCohereEmbeddings(BaseModel, Embeddings):
    """
    ...
    def embed_with_retry(self, **kwargs: Any) -> Any:
        ...
        for raw_input in kwargs.get("texts"):
            if is_base64_data_uri(raw_input):
                ...
                embed_input = EmbedInput(content=[TextEmbedContent(text=raw_input)])
                embed_inputs.append(embed_input)
